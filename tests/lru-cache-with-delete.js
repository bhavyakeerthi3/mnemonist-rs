...
function LRUCacheWithDelete(keyType, valueType, capacity) {
  let cap = capacity;
  ...
  if (typeof cap !== 'number' || cap <= 0 || !Number.isInteger(cap) || cap === Infinity) {
    throw new Error('LRUCacheWithDelete: capacity must be a positive integer.');
  }
...
LRUCacheWithDelete.prototype.peek = function(key) {
  if (protocol) return this._inner.callOpaque('peek', [key]);
  if (!this._inner.has(key)) return undefined;
  return this._values.get(key);
};

LRUCacheWithDelete.prototype.has = function(key) {
...
LRUCacheWithDelete.prototype.remove = function(key, missingMarker) {
  if (protocol) {
...
LRUCacheWithDelete.prototype[Symbol.iterator] = function() {
  return this.entries();
};

Object.defineProperty(LRUCacheWithDelete.prototype, 'size', {
  get: function() { return protocol ? this._inner.size() : this._inner.size; }
});

Object.defineProperty(LRUCacheWithDelete.prototype, 'capacity', {
  get: function() { return protocol ? this._inner.call('capacity', []) : this._inner.capacity; }
});

Object.defineProperty(LRUCacheWithDelete.prototype, 'head', {
  get: function() {
...
Object.defineProperty(LRUCacheWithDelete.prototype, 'tail', {
  get: function() {
    if (this.size === 0) return 0;
    const entries = protocol ? this._inner.call('entries', []) : this._inner.entries();
...
Object.defineProperty(LRUCacheWithDelete.prototype, 'items', {
  get: function() {
    const obj = {};
    const entries = protocol ? this._inner.call('entries', []) : this._inner.entries();
...
LRUCacheWithDelete.from = function(iterable, keyType, valueType, capacity) {
  let cap = capacity;
  ...
  const cache = new LRUCacheWithDelete(cap);
